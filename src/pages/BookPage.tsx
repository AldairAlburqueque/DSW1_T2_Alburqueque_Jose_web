...
const BooksPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMessage('error', err.response?.data?.message || 'Error al crear libro');
    }
  };
    ...
    <div className="page">
      ...
      <div className="form-section">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <label>Stock</label>
            <input type="number" name="stock" value={formData.stock} min={1} onChange={handleChange} required />
          </div>
          <button type="submit" className="btn btn-primary">Guardar</button>
          <button type="button" className="btn btn-secondary" onClick={clearForm}>Limpiar</button>
        </form>
      </div>
